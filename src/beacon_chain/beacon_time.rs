...
pub fn date_time_from_slot(slot: &u32) -> DateTime<Utc> {
...
pub fn get_is_first_of_day(slot: &u32) -> bool {
    match slot {
        slot if *slot == 0 => true,
        slot if *slot > 0 => {
            let day_of_month_previous_slot = date_time_from_slot(&(slot - 1)).day();
            let day_of_month = date_time_from_slot(&slot).day();
