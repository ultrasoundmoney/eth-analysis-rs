...
use eth_analysis::{
    ...
    db,
    effective_balance_sum::{self, EffectiveBalanceSum},
    log,
};
...
pub async fn main() {
    ...
    let sum =
        effective_balance_sum::get_effective_balance_sum(&beacon_node, &last_state.state_root)
            .await;

    effective_balance_sum::store_effective_balance_sum(&db_pool, &last_state.state_root, &sum)
        .await;
